...
async function renderDashboard() {
  ...
  el.innerHTML = `
    <div class="page-header">
      <h1>Dashboard</h1>
      <span style="color:#888;font-size:14px">${year}</span>
    </div>
    ...
    ${vehicles.length === 0 ? `
      <div class="empty-state">
        ...
        <button class="btn btn-primary" onclick="showVehicleForm(null)" style="margin-top:16px">+ Add Vehicle</button>
      </div>
    ` : `
      ${nextServiceHtml}
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value">${totalKm > 0 ? Math.round(businessKm / totalKm * 100) : 0}%</div>
          <div class="stat-sub">of total km</div>
        </div>
      </div>
...
function tripRowHTML(t, vehicle) {
  ...
  return `
    <div class="trip-row" onclick="navigate('logTrip', { editId: ${t.id} })">
      <div class="trip-main">
        <div class="trip-route">${t.start_location} &rarr; ${t.end_location}${gpsIcon}</div>
        <div class="trip-meta">${t.purpose} ${vName ? '&bull; ' + vName : ''}</div>
      </div>
      <div class="trip-km">${Number(t.km_driven).toFixed(1)} km<br><small>${formatDate(t.date)}</small></div>
    </div>
  `;
}
