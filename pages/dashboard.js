    ...
    if (withDue.length > 0) {
      ...
      nextServiceHtml = `
          <div class="maint-due-card maint-due-${urgency}" onclick="navigate('maintenance')" style="cursor:pointer;">
            <div class="maint-due-label"><i data-lucide="wrench"></i> Next ${latest.type}</div>
  ...
  el.innerHTML = `
    ...
    ${vehicles.length === 0 ? `
        <div class="empty-state">
          <div style="display:flex;justify-content:center"><i data-lucide="car" style="width:48px;height:48px"></i></div>
          <div class="empty-title">No vehicles yet</div>
          <div class="empty-sub">Add your first vehicle to get started</div>
          <button class="btn btn-primary" onclick="showVehicleForm(null)"><i data-lucide="plus"></i> Add Vehicle</button>
        </div>
    ` : `
      ${nextServiceHtml}
      <div class="stat-grid">
      ...
      <div class="quick-actions">
        <button class="btn btn-primary" onclick="navigate('logTrip')"><i data-lucide="map-pin-plus"></i> Log Trip</button>
        <button class="btn btn-secondary" onclick="navigate('logFuel')"><i data-lucide="fuel"></i> Log Fuel</button>
        <button class="btn btn-secondary" onclick="navigate('maintenance', {showForm:true})"><i data-lucide="wrench"></i> Service</button>
        <button class="btn btn-secondary" onclick="navigate('expenses')"><i data-lucide="bar-chart-2"></i> Expenses</button>
      </div>
      ${yearTrips.length > 0 ? `
...
function tripRowHTML(t, vehicle) {
  ...
  const gpsIcon = t.gps_tracked ? ' <i data-lucide="map-pin"></i>' : '';
  return `
